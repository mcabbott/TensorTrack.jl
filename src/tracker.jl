using Tracker
using Tracker: track, @grad, TrackedArray, TrackedReal, data

# similar_from_indices always makes an un-tracked array, as tracking is handled outside its concerns

similar_from_indices(::Type{Tracker.TrackedReal{T}}, p1::IndexTuple, p2::IndexTuple, A, CA::Symbol) where T =
    similar_from_indices(T, p1, p2, data(A), CA)

cached_similar_from_indices(sym::Symbol, ::Type{Tracker.TrackedReal{T}}, p1::IndexTuple, p2::IndexTuple, A, CA::Symbol) where T =
    cached_similar_from_indices(sym, T, p1, p2, data(A), CA)

similar_from_indices(::Type{Tracker.TrackedReal{T}}, poA::IndexTuple, poB::IndexTuple,
    p1::IndexTuple, p2::IndexTuple, A, B, CA::Symbol, CB::Symbol) where T =
    similar_from_indices(T, poA, poB, p1, p2, data(A), data(B), CA, CB)

cached_similar_from_indices(sym::Symbol, ::Type{Tracker.TrackedReal{T}}, poA::IndexTuple, poB::IndexTuple,
    p1::IndexTuple, p2::IndexTuple, A, B, CA::Symbol, CB::Symbol) where T =
    cached_similar_from_indices(sym, T, poA, poB, p1, p2, data(A), data(B), CA, CB)

StridedView(A::Tracker.TrackedArray) = StridedView(A.data)
UnsafeStridedView(A::Tracker.TrackedArray) = UnsafeStridedView(A.data)
